...
.stop_input <- function(routine, param, msg, call = sys.call(-2L)) {
  stop(pladdrr_error_cond("pladdrr_input_error", routine, param, msg, call = call))
}
...
.check_positive_count <- function(value, name) {
  if (!is.numeric(value) || length(value) != 1L || is.na(value) || value < 1) {
    .stop_input("check_positive_count", name,
                paste0(name, " must be a single number >= 1, got: ", deparse(value)))
  }
  invisible(TRUE)
}
...
# Praat's "Robust slow" trend fit (kCepstrum_trendFit::ROBUST_SLOW -> SlopeSelector
# THEILSEN) draws random samples via NUMrandomInteger inside Matousek's slope
# selection, and the upstream implementation is not reproducible: repeated runs on
# identical input differ by ~0.8 dB, and Praat itself occasionally returns values on
# the order of 1e290. pladdrr inherits the instability faithfully. Warn once per
# session so a user does not silently build an analysis on an irreproducible number.
...
.check_quefrency_range <- function(qstart, qend, qstart_name = "qstart_fit",
  ...
  if (qend != 0 && qend <= qstart) {
    .stop_input("check_quefrency_range", qend_name,
                paste0(qend_name, " (", qend, ") must be greater than ", qstart_name,
...
# To Pitch / To Intensity form, so 0 must be accepted here too — rejecting it made
# a documented Praat idiom unusable in to_formant_keepall/willems/sl. Negative or
...
# outside the signal and returns it silently, so `sound$extract_part(5, 10)` on a
# 1 s Sound yields 5 s of fabricated silence. pladdrr reproduces the value (goal 1)
...
.check_extract_range <- function(.self, from_time, to_time) {
  ...
  if (lo < xmin || hi > xmax) {
    .warn_data_loss(
      "sound_extract_part",
      sprintf(paste0("requested [%g, %g] s extends outside the signal [%g, %g] s; ",
