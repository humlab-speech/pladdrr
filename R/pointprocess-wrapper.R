# pointprocess-wrapper.R - PointProcess object using shared dispatch table
#  (pladdrr 4.8.33)
...
.pp_methods$get_number_of_points <- function(
  .self) .self$.cpp$get_number_of_points()
.pp_methods$get_time_from_index <- function(.self,
  index) .self$.cpp$get_time(as.integer(index))
.pp_methods$get_time <- function(.self,
  index) .self$.cpp$get_time(as.integer(index))
.pp_methods$get_nearest_index <- function(.self,
  time) .self$.cpp$get_nearest_index(as.numeric(time))
.pp_methods$get_low_index <- function(.self,
  time) .self$.cpp$get_low_index(as.numeric(time))
.pp_methods$get_high_index <- function(.self,
  time) .self$.cpp$get_high_index(as.numeric(time))
.pp_methods$get_interval <- function(.self,
  time) .self$.cpp$get_interval(as.numeric(time))
...
.pp_methods$remove_points_between <- function(.self, from_time, to_time) {
  .self$.cpp$remove_points_between(as.numeric(from_time),
    as.numeric(to_time)); invisible(.self)
}
.pp_methods$fill <- function(.self, from_time, to_time, period) {
  .self$.cpp$fill(as.numeric(from_time), as.numeric(to_time),
    as.numeric(period)); invisible(.self)
}
.pp_methods$voice <- function(.self, period, max_period_factor = 1.3) {
  .self$.cpp$voice(as.numeric(period),
    as.numeric(max_period_factor)); invisible(.self)
}
...
# --- Batch operations ---
.pp_methods$get_values_from_sound <- function(.self, sound, channel = 1,
  interpolation = "cubic") {
...
.pp_methods$get_periods_vector <- function(
  .self) .self$.cpp$get_periods_vector()
.pp_methods$get_periods_filtered <- function(.self, min_period = 0.0001,
  max_period = 0.02) {
  .self$.cpp$get_periods_filtered(as.numeric(min_period),
...
.pp_methods$get_jitter_local_absolute <- function(.self, from_time = 0,
  to_time = 0,
    period_floor = 0.0001, period_ceiling = 0.02,
    max_period_factor = 1.3) {
  if (!is.null(.self$.my_scache)) {
...
.pp_methods$get_shimmer_local <- function(.self, sound, from_time = 0,
  to_time = 0,
    period_floor = 0.0001, period_ceiling = 0.02,
...
.pp_methods$to_textgrid_vuv <- function(.self, max_voiced_period,
  max_unvoiced_period = 0.02) {
  tg_ptr <- .pointprocess_to_textgrid_vuv(.self$.xptr,
    as.numeric(max_voiced_period), as.numeric(max_unvoiced_period))
...
PointProcess <- function(tmin = NULL, tmax = NULL, .xptr = NULL) {
  ...
  if (!is.null(.xptr)) {
    ...
    return(
      structure(list(.xptr = .xptr, .cpp = cpp_pp),
