...
**Should you use them?** Yes! They're equivalent to Direct API functions and provide the same 2-3x speedup.

**Equivalence:**
```r
# These are functionally equivalent patterns:

# Pattern 1: Using _fast functions
cpps <- calculate_cpps_fast(sound)

# Pattern 2: Using _direct equivalent
...
- 5-10x faster than loops
- Minimize R↔C++ boundary crossings

#### 3b. Vectorized Queries

**Naming:** `get_*_at_times()` or `*_get_values_at_times()` (deprecated)

**Pattern:** `values <- get_property_at_times(object, times)`

**Location:** `R/batch-queries.R`, `R/batch-ops.R` (deprecated versions)

**Examples:**
```r
# Preferred (batch-queries.R)
...
# Deprecated (batch-ops.R) - shows warnings
f0_values <- pitch_get_values_at_times(pitch, times)  # Use get_pitch_at_times()
f1_values <- formant_get_values_at_times(formant, times, 1)  # Use get_formants_at_times()
```

**Characteristics:**
- Query multiple time points efficiently
- 10-20x faster than loops
...
- 3-8x faster on multi-core systems
...
- **Deprecated:** `*_get_values_at_times()` functions (use `get_*_at_times()`)
...
**Naming:** `*_zerocopy` or `get_*_fast` (legacy)

**Purpose:** Direct memory access without copying

**Examples:**
```r
values <- get_sound_values_zerocopy(sound)
matrix <- sound_as_matrix_zerocopy(sound)
```
...
|------|----------------|----------|----------|
| `sound-r6-new.R` | Object methods | Tier 1 | `$to_pitch()`, `$to_formant()` |
...
| `zerocopy-access.R` | `*_zerocopy` | Tier 3+ | `get_sound_values_zerocopy()` |
...
1. **Remove deprecated functions**
   - `pitch_get_values_at_times()` removed
   - `formant_get_values_at_times()` removed
   - `intensity_get_values_at_times()` removed

2. **Possible renaming (under consideration)**
   ...
   **Decision:** Likely keep `_fast` for backward compatibility unless user feedback demands change.
...
| Pattern | Purpose | Speed | Examples |
|---------|---------|-------|----------|
| `$method()` | Standard API | 1x | `sound$to_pitch()` |
| `*_direct()` | Skip dispatch | 2-3x | `to_pitch_direct()` |
| `*_fast()` | Legacy direct | 2-3x | `calculate_cpps_fast()` |
| `*_batch()` | Multi-item | 5-10x | `sound_to_pitch_batch()` |
| `get_*_at_times()` | Vectorized query | 10-20x | `get_pitch_at_times()` |
| `*_parallel()` | Multi-core | 3-8x | `extract_pitch_parallel()` |
| `*_and_*()` | Combined ops | 5-10x | `sound_extract_and_pitch()` |
...
3. Look at examples in the batch operations guide: `BATCH_OPERATIONS_GUIDE.md`
4. Follow the pattern of similar functions

**General rule:** If a function has a performance-related suffix (`_direct`, `_fast`, `_batch`, `_parallel`, `_at_times`), it's optimized for speed and follows the patterns described in this document.
