...
- **v4.9.15 — CPPS slopeselector SIMD gate + stale-binary caveat + build-flag guidance (downstream perf assessment 2026-07-31):**
  - **`should_use_simd_for_slopeselector()` now honors two env knobs** (`src/slopeselector_simd.cpp`): `PLADDRR_DISABLE_SLOPESELECTOR_SIMD=1` forces scalar, `PLADDRR_ENABLE_SLOPESELECTOR_SIMD=1` forces SIMD. Default stays **ON** — on a clean build the vectorized Siegel trend fit (commit `6a63af59`) is ~25% faster than scalar even on arm64/NEON (CPPS `ppq1.wav`: SIMD 0.92s vs scalar 1.23s), bit-identical output. The commit's speedup claim holds.
  - **⚠️ Stale-binary trap (root-caused downstream).** A released `.so` can carry a **mis-optimized `slopeselector_simd.o`** that is ~2× *slower* than a clean recompile of identical source (observed SIMD 2.14s vs 0.92s; CPP tool 3.63s vs 1.55s; AVQI 4.53s vs 0.69s). The scalar path is stable across builds — only the SIMD object regresses when its optimization flags don't reach that TU (build-system fragility, cf. the v4.8.32 `configure`-regenerates-`Makevars` note). **If CPPS/AVQI timing regresses, force a clean rebuild of this TU (`R CMD INSTALL --preclean .`) before suspecting the algorithm or disabling SIMD.** Do NOT reflexively gate SIMD off on arm64 — that was tested and is a 25% pessimization on a good build.
  ...
  - **Build-flag guidance for non-CRAN consumers.** The measured cost of the CRAN `-O2` build is **much larger** than the "modest cepstrogram/CPPS gain" this guide previously stated: a local rebuild with `~/.R/Makevars` `CFLAGS/CXXFLAGS/CXX17FLAGS = -O3 -march=native` (keeping `-ffp-contract=off`; **never** `-ffast-math`) yields **3–7× on nearly every routine with zero faithfulness loss** (downstream plabench: 15/15 3-way Praat-parity tests stay bit-exact; AVQI 4.53s→0.69s, DSI 0.27s→0.06s, Formant 0.11s→0.04s). `CXX17FLAGS` is required — R applies it (not `CXXFLAGS`) to `CXX_STD=CXX17` TUs. CRAN/CI must stay `-O2`; this is a deploy-time option for embedders, not a package default.
