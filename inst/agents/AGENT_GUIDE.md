...
- **v5.0.4 — 8 Spectrogram/PowerCepstrogram/PowerCepstrum plotting bugs fixed; all 5 affected functions had zero prior test coverage (2026-08-19).**
  ...
  - **Deliberately not fixed in this release** (documented, not silently dropped):
    - The C++-level column mislabel itself (`RPowerCepstrum::as_data_frame()`, `src/modules/powercepstrum_module.cpp:246-256`) is untouched — this release fixed one more R-layer call site that reads from it (see `POWERCEPSTRUM_DB_MISLABEL.md`, updated); `autoplot.PowerCepstrum()`/`autolayer.PowerCepstrum()` (`R/autoplot-methods.R` ~555-609) and `plot.PowerCepstrum()` (`R/plotting-methods.R` ~900-935) still have the unconverted-dB bug and are still open.
    - `plot_powercepstrogram(..., show_cpp_contour = TRUE)`'s CPP-contour overlay still draws a flat, meaningless line at a hardcoded `quefrency = 0.01` (`R/cepstrum_plots.R`) regardless of where the actual cepstral peak is — needs a real per-time-point peak-quefrency query, which is more design work than this bugfix pass scoped for.
    - `plot.Spectrogram()`'s `preemphasis` parameter (default `50`) is accepted but never referenced in the function body — either dead code or an unimplemented pre-emphasis filter; needs a decision, not a mechanical fix.
    - `plot_cpp_timeseries()`'s `labs(subtitle = ...)` still computes `mean()`/`sd()` on a possibly-0-row `plot_data$cpp` a second time (redundant with the now-guarded main annotation), producing a cosmetic "Mean CPP: NaN dB (SD: NA)" subtitle in the all-samples-failed case — no warning, no error, just a cosmetic string.
