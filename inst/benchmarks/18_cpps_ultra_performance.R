...
# Tracks calculate_cpps_ultra() timing on the DSI reference file, and
# cross-checks fidelity vs the known-good Praat-matching values. See
# PLADDRR_CPPS_PERF_SPEC.md
...
wav_path <- system.file(
  "signalfiles", "DSI", "input", "ppq1.wav",
  package = "pladdrr"
)
if (!nzchar(wav_path)) {
...
# PLADDRR_CPPS_PERF_SPEC.md). robust1 is the default ROBUST_FAST/straight
# fit; lsq is the LEAST_SQUARES/straight fit.
reference <- list(
...
if (abs(robust1 - reference$robust1) > 1e-9 ||
      abs(lsq - reference$lsq) > 1e-9) {
...
# elapsed_s: convert ms to s
elapsed_s <- summary(timing)$mean / 1000
cat(sprintf(
...
# The short ppq1 case above hid the v4.9.15 arm64 SIMD regression: the
# per-frame Siegel trend fit only dominates on many-frame (long) signals.
# Concatenate ppq1 to ~8x length so the trend-fit hot path is exercised
# at scale. The default gate is scalar on arm64 (see
# slopeselector_simd.cpp); force-enable via
# PLADDRR_ENABLE_SLOPESELECTOR_SIMD=1 in a *separate* R process to A/B
# (the gate is read once at static init, so it cannot be toggled
# mid-session).
cat("\n=== Long-signal case (~8x ppq1) ===\n")
long <- sound_concatenate_all(rep(list(s), 8))
cat(sprintf(
  "Long signal duration: %.2f s\n",
...
cat(sprintf(
  paste0(
    "\nLong-signal mean warm elapsed: %.3f s ",
    "(slopeselector SIMD force-enable=%s)\n"
  ),
  long_elapsed_s, simd_state
))
cat(
  "Compare across pladdrr versions / arches; ",
  "a >20%% jump here is a trend-fit regression.\n",
  sep = ""
)
