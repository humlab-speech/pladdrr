...
#include "praat.github.io/fon/TextGrid_def.h"
}
...
Rcpp::XPtr<structTextGrid> textgrid_read_from_file(std::string path) {
    return praat_try_catch<structTextGrid>([&]() {
        structMelderFile file = {};
        Melder_relativePathToFile(Melder_peek8to32(path.c_str()), &file);
        autoTextGrid textgrid = TextGrid_readFromFile(&file);
        return textgrid.releaseToAmbiguousOwner();
    }, "Failed to read TextGrid from file");
}
...
std::string textgrid_get_tier_name(Rcpp::XPtr<structTextGrid> xptr, int tier_number) {
    return praat_try_catch<std::string>([&]() {
        if (!xptr) Rcpp::stop("Invalid TextGrid pointer");
...
std::vector<std::string> textgrid_get_tier_names(Rcpp::XPtr<structTextGrid> xptr) {
    return praat_try_catch<std::vector<std::string>>([&]() {
        if (!xptr) Rcpp::stop("Invalid TextGrid pointer");
...
Rcpp::XPtr<structTextGrid> textgrid_extract_part(
    ...
    return praat_try_catch<structTextGrid>([&]() {
        ...
        return extracted.releaseToAmbiguousOwner();
    }, "Failed to extract part");
}
...
Rcpp::DataFrame textgrid_to_data_frame(
    ...
    return praat_try_catch<Rcpp::DataFrame>([&]() {
        if (!xptr) Rcpp::stop("Invalid TextGrid pointer");
