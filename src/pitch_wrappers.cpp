...
Rcpp::XPtr<structTextGrid> pitch_to_textgrid_vuv(
    ...
    try {
        ...
        // Clear default intervals
        tier->intervals.removeAllItems();
        
        // Track voiced/unvoiced state
        double start_time = pitch->xmin;
        bool prev_voiced = false;
        double prev_time = start_time;
        
        for (integer iframe = 1; iframe <= pitch->nx; iframe++) {
            double time = Sampled_indexToX(pitch.get(), iframe);
            double freq = Pitch_getValueAtTime(pitch.get(), time, kPitch_unit::HERTZ, false);
            bool is_voiced = isdefined(freq) && freq > 0.0;
            
            // State change or final frame
            if (iframe == 1) {
                prev_voiced = is_voiced;
                prev_time = start_time;
            } else if (is_voiced != prev_voiced || iframe == pitch->nx) {
                // Add interval for previous state
                double end_time = (iframe == pitch->nx) ? pitch->xmax : time;
        ...
        // Ensure final interval
        if (prev_time < pitch->xmax) {
            autoTextInterval interval = TextInterval_create(
                prev_time,
                pitch->xmax,
                prev_voiced ? U"V" : U"U"
...
Rcpp::XPtr<structTextGrid> pitch_to_textgrid_silences(
    ...
    try {
        // Silences are unvoiced frames
        // This is a simplified version - full implementation would use intensity
        autoTextGrid tg = TextGrid_create(
            pitch->xmin,
            pitch->xmax,
            U"silences", U""
        ...
        double start_time = pitch->xmin;
        bool is_silent = true;
        double silent_start = start_time;
        
        for (integer iframe = 1; iframe <= pitch->nx; iframe++) {
            double time = Sampled_indexToX(pitch.get(), iframe);
            double freq = Pitch_getValueAtTime(pitch.get(), time, kPitch_unit::HERTZ, false);
            bool frame_silent = !isdefined(freq) || freq <= 0.0;
            
            if (iframe == 1) {
                is_silent = frame_silent;
                silent_start = start_time;
            } else if (frame_silent != is_silent) {
                // State changed
                double duration = time - silent_start;
                bool add_interval = is_silent ? 
                    (duration >= min_silent_duration) : 
                    (duration >= min_sounding_duration);
                
                if (add_interval) {
                    autoTextInterval interval = TextInterval_create(
                        silent_start,
                        time,
                        is_silent ? U"silent" : U"sounding"
                    );
                    tier->intervals.addItem_move(interval.move());
                }
        ...
        // Final interval
        double duration = pitch->xmax - silent_start;
        bool add_final = is_silent ? 
            (duration >= min_silent_duration) : 
            (duration >= min_sounding_duration);
        ...
        Melder_clearError();
        Rcpp::stop("Failed to create silence TextGrid from Pitch");
    }
}

// ============================================================================
// TextGrid conversion methods
// ============================================================================
