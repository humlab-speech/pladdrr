...
This guide helps existing pladdrr users migrate to the latest API
improvements, particularly for heavy users with production code.
...
However, some functions are now deprecated and will show warnings.
Update your code to use the preferred alternatives.
...
|----|----|----|
| `pitch_get_values_at_times()` | [`get_pitch_at_times()`](https://humlab-speech.github.io/pladdrr/reference/get_pitch_at_times.md) | Will be removed in v3.0.0 |
| `formant_get_values_at_times()` | [`get_formants_at_times()`](https://humlab-speech.github.io/pladdrr/reference/get_formants_at_times.md) | Will be removed in v3.0.0 |
| `intensity_get_values_at_times()` | [`get_intensity_at_times()`](https://humlab-speech.github.io/pladdrr/reference/get_intensity_at_times.md) | Will be removed in v3.0.0 |
...
**Why?** The `get_*_at_times()` naming is more consistent with the rest
of the API.
...
PowerCepstrogram was converted from R6 class to function wrapper for
consistency.
...
If you work with external pointers directly, the extraction pattern has
been standardized.
...
# Use the standardized utility (internal, not exported - call with :::)
ptr <- pladdrr:::extract_xptr(obj, "Sound")
...
The batch operations in `batch-ops.R` have been fixed to handle modern
objects correctly. No code changes needed if you just call the batch
functions.
...
**Notes:** - Wraps your existing per-file analysis code with no other
changes - Automatic platform detection (Unix/Windows) - Auto-detects
optimal core count

#### New Direct API Functions (v2.3.0)

Four new direct conversion functions that skip R6 object dispatch:
...
returns a list with F1, F2, F3, F4:
...
**Opportunity:** If you process \>50 files, consider parallel
processing.
...
unit_code <- pladdrr:::unit_to_code("hertz", "pitch")  # Returns 0L
unit_code <- pladdrr:::unit_to_code("mel", "pitch")     # Returns 2L
unit_code <- pladdrr:::unit_to_code("bark", "formant")  # Returns 1L
```
...
# Extract and analyze, one interval at a time
...
# Combined extract + analyze in one call
...
# analysis_func receives only a Sound object, not its source path, so pair
# the results back up with `files` afterwards (or with names(results)).
results <- analyze_files_parallel(files, function(sound) {
...
# After (v2.3.0) - Vectorized, one R->C crossing for the whole vector
...
2.  **Look for deprecation warnings** - They tell you exactly what to
    change

3.  **Test with small datasets first** - Verify changes work before
    running on full corpus

4.  **Measure your own workload** - Use
    [`system.time()`](https://rdrr.io/r/base/system.time.html)/`microbenchmark`
    to see whether a change helps your pipeline
...
- **v2.4.0 (TBD):** Deprecated functions still work but show louder
  warnings
- **v3.0.0 (TBD):** Deprecated functions removed

**Recommendation:** Update your code now to avoid warnings and prepare
for v3.0.0.
...
|----|----|----|
| `pitch_get_values_at_times()` | [`get_pitch_at_times()`](https://humlab-speech.github.io/pladdrr/reference/get_pitch_at_times.md) | Rename only |
| `formant_get_values_at_times()` | [`get_formants_at_times()`](https://humlab-speech.github.io/pladdrr/reference/get_formants_at_times.md) | Rename + access `$F1`, `$F2` |
| `intensity_get_values_at_times()` | [`get_intensity_at_times()`](https://humlab-speech.github.io/pladdrr/reference/get_intensity_at_times.md) | Rename only |
| `PowerCepstrogram$new()` | [`PowerCepstrogram()`](https://humlab-speech.github.io/pladdrr/reference/PowerCepstrogram.md) | Remove `$new` |

#### New Features to Adopt

| Feature | Function | What It Does |
|----|----|----|
| Parallel processing | [`analyze_files_parallel()`](https://humlab-speech.github.io/pladdrr/reference/analyze_files_parallel.md) | Distributes file-level work across cores |
| Direct API | `to_*_direct()` | Skips R6 object dispatch |
| Batch stats | [`get_pitch_stats_direct()`](https://humlab-speech.github.io/pladdrr/reference/get_pitch_stats_direct.md) | Returns all statistics in one call |
| Combined ops | [`sound_extract_and_pitch()`](https://humlab-speech.github.io/pladdrr/reference/sound_extract_and_pitch.md) | Combines extraction and analysis in one call |
...
2.  **Update deprecated functions** - simple renames
3.  **Adopt new APIs** - parallel processing and Direct API skip R6
    dispatch overhead
4.  **Read new docs** - see
    [`vignette("performance-optimization")`](https://humlab-speech.github.io/pladdrr/articles/performance-optimization.md)
