...
KlattGrid is pladdrr’s implementation of the Klatt cascade/parallel
formant synthesizer, for generating synthetic speech from parametric
descriptions. Unlike recording or playing back audio, KlattGrid creates
speech from parameters by specifying:
...
1.  **Independent control** - Change F1 independently of F2, not
    possible in human speech
2.  **Parametric variation** - Generate continua (e.g., /i/ to /u/ in 10
    steps)
...
Dennis Klatt’s 1980 formant synthesizer is a cascade/parallel model of
parametric speech synthesis. It models the vocal tract as a cascade of
resonant filters (formants) excited by a periodic source (voicing) or
noise.
...
(fundamental frequency) - Voicing amplitude tier (phonation) - All
formant frequency tiers (F1, F2, F3, …) - All formant bandwidth tiers
(B1, B2, B3, …)
...
**Current limitations:** - Cannot query tier contents directly - Cannot
remove points after adding - No direct formant amplitude control

**Best practice:** Create new grid with desired parameters rather than
modifying existing grid

### 3. Formant Constraints

**Physical constraints that must be maintained:** - F1 \< F2 \< F3
(formants increase monotonically) - Bandwidths \> 0 Hz (zero bandwidth
causes artifacts) - Pitch in range 50-400 Hz (outside this sounds
unnatural)
...
1.  **Don’t use empty
    [`KlattGrid()`](https://humlab-speech.github.io/pladdrr/reference/KlattGrid.md)
    constructor** (errors on synthesis)
2.  **Don’t violate F1 \< F2 \< F3** (unnatural synthesis)
3.  **Don’t use zero bandwidth** (causes artifacts)
4.  **Don’t expect consonant synthesis** (formants only, no articulation
    model)
...
**Solution**: Ensure `f0start` is 80-300 Hz and voicing amplitude \> 40
dB
...
Parselmouth does not currently expose KlattGrid - pladdrr is the only R
package with direct Klatt synthesis support.

### vs Other R Synthesis

Existing R packages (e.g., `phonTools`) use simpler synthesis methods.
pladdrr’s KlattGrid wraps Praat’s own cascade/parallel Klatt synthesizer
C++ code directly, rather than reimplementing it.

## Summary

**KlattGrid is used for:** - Perceptual experiments requiring controlled
stimuli - Voice manipulation research - Teaching phonetic concepts -
Speech resynthesis with modified parameters
...
**Typical workflow:** 1. Define formant parameters (F1/F2/F3, B1/B2/B3)
2. Set pitch contour (f0start + optional points) 3. Create grid with
`createFromVowel()` 4. Synthesize with `to_sound()` 5. Analyze or export
