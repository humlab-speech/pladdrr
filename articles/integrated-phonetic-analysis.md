...
This vignette demonstrates how to perform integrated phonetic analysis
workflows using the **pladdrr** package. We will cover:
...
This workflow is typical in phonetic research, where annotated speech
data requires systematic acoustic analysis.

## Background

### TextGrid Annotation

TextGrids are Praat’s standard format for time-aligned annotations. They
consist of:

- **Interval tiers**: Segments with start/end times and labels (e.g.,
  words, phones)
...
- Multi-level linguistic annotation (phones, syllables, words,
  utterances)
...
For demonstration, we create a 5-second audio signal. In real research,
you would load existing recordings.
...
p <- ggplot2::autoplot(tg)
p2 <- p + ggplot2::autolayer(tg, tier = 2, color = "darkred")
class(p2)
...
#> Tier 2: phones (IntervalTier) - 9 items
```
...
#> Applied pre-emphasis from 50 Hz
```
...
cat(sprintf("Formants at %.2f s: F1 = %.0f Hz, F2 = %.0f Hz, F3 = %.0f Hz\n",
            vowel_time, f1, f2, f3))
#> Formants at 1.50 s: F1 = 404 Hz, F2 = 443 Hz, F3 = 481 Hz
```
...
spaces, formant trajectories, pitch/intensity contours, spectrograms,
and voice quality reports, see
...
f0 <- pitch$get_value_at_time(time, "hertz", interpolate = TRUE)
if (is.na(f0)) {
...
- [`vignette("textgrid-workflows")`](https://humlab-speech.github.io/pladdrr/articles/textgrid-workflows.md) -
  Advanced TextGrid manipulation
