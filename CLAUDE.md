...
R package: OO interface to Praat phonetic analysis via Rcpp. Wraps Praat C++ (from `praat.github.io/`) with R6 classes.
...
- `to_point_process_direct()` arg order: `(sound, pitch_floor, pitch_ceiling, time_step=0, max_period_factor, max_amplitude_factor)`
- `get_jitter_shimmer_batch()` returns all 6 shimmer metrics in one C++ call
...
| `get_cpps_fast()` (AVQI) | 0.01 | 0.001 | 333.3 | straight (1) |
