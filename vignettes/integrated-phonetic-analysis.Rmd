...
- **Interval tiers**: Segments with start/end times and labels (e.g., words,
  phones)
...
`autoplot(
  )` and `autolayer()` methods let you compose TextGrid tier displays with ggplot2:
...
`pladdrr` provides `autoplot(
  )`/`autolayer()` methods (shown above for `TextGrid`) plus `plot()` methods for Sound, Pitch, Formant, Intensity, and other analysis objects. For vowel spaces, formant trajectories, pitch/intensity contours, spectrograms, and voice quality reports, see `vignette("visualization")`.
...
**Note:** CPPS values are sensitive to parameters. See `system.file(
  "examples", "10_cpps_analysis.R", package = "pladdrr")` for parameter effects and clinical interpretation.
...
- **Multi-dimensional acoustic analysis** (prosody, voice quality,
  spectral features)
- **R integration** for statistical analysis and visualization via `data.frame` output and `autoplot(
...
- Thomas, E. R., & Kendall, T. (
  2007). *NORM: The vowel normalization and plotting suite*.
