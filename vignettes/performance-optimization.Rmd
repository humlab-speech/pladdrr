...
│ - Speed: 5-20x faster for batch processing              │
└─────────────────────────────────────────────────────────┘
```
...
stats <- get_pitch_stats_direct(pitch)  # 2-3x faster
```
...
## Tier 3: Batch & Parallel API (5-20x Faster)
...
pitches <- sound_to_pitch_batch(sounds)  # 1 R→C crossing, 5-10x faster
```
...
f0_values <- pitch_get_values_at_times(pitch, times)  # 10x faster
```

**Vectorized query functions:**
- `pitch_get_values_at_times()` - Batch pitch queries
- `formant_get_values_at_times()` - Batch formant queries
- `intensity_get_values_at_times()` - Batch intensity queries
...
# ~4 seconds (3.75x faster, includes file I/O)
```
...
values <- pitch_get_values_at_times(pitch_ptr, times)
```
...
analyze_files_parallel(files, function(sound) {
  ...
  lapply(formants, function(f) {
    formant_get_values_at_times(f, analysis_times)
  })
}, n_cores = 4)
```
