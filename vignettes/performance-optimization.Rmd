...
```{r best_practice3}
# Avoid: Extract pointer every iteration
for (time in times) {
  val <- get_pitch_value_direct(pitch, time)
}

# Prefer: Extract once, use a vectorized query
...
benchmark_results <- benchmark_parallel(
  files[1:20],  # Use subset for a quick test
...
```{r best_practice5}
# Parallel processing with batch operations inside each worker
