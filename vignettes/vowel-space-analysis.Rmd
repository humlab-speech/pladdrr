---
title: "Vowel Space Analysis with speaker"
author: "speaker Package Authors"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Vowel Space Analysis with speaker}
...
This vignette demonstrates a complete vowel acoustics research workflow using the **speaker** package. Topics covered:
...
This workflow is essential for sociolinguistic variation studies, L2 acquisition research, and dialectology.
...
For this demonstration, we'll use synthetic data. In real research, load actual recordings:
...
for (i in 1:nrow(vowel_segments)) {
  tg$set_interval_label(1, i + 1, vowel_segments$vowel[i])
}
...
```{r preemphasis}
sound_preemph <- sound$pre_emphasize(from_frequency = pre_emphasis)
```
...
For multi-speaker data with repeated measures:
...
```{r parameter_testing, eval=FALSE}
# Test multiple ceilings
ceilings <- c(5000, 5500, 6000, 6500)
for (ceiling in ceilings) {
  formant <- sound$to_formant_burg(maximum_formant = ceiling, ...)
...
The **speaker** package provides a complete vowel acoustics pipeline:

- **Flexible formant extraction** with validated Praat algorithms
- **Multi-point measurements** for trajectory analysis
- **Built-in normalization** (or bring your own)
- **Vowel space metrics** (area, dispersion)
- **Seamless R integration** for analysis and visualization
