...
**Known issue**: in pladdrr 5.0.0, calling `extract_formant()` on a
`FormantPath` object after calling `set_path()` or `set_optimal_path()`
crashes the R session in this build (verified by direct testing, not just an
R-level error). Until this is fixed upstream, avoid extracting from a path
you have manually edited this way. `path_finder()` (below), which also
rewrites the path, does not exhibit this problem.
