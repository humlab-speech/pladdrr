...
- **Cepstral analysis**: `plot_powercepstrum(
  )`, `plot_powercepstrogram()`, `plot_cpp_timeseries()`, `create_cepstrum_report()`
...
- **Prosodic features**: pitch and intensity contours from `Pitch$as_data_frame(
  )` and `Intensity$as_data_frame()`
- **Annotation**: TextGrid tier display via `autoplot.TextGrid(
  )` and `autolayer.TextGrid()`
- **Spectral analysis**: spectrograms, spectra, and LTAS from the corresponding `as_matrix(
...
- `vignette(
  "autoplot-autolayer")` - Full reference for the `autoplot`/`autolayer` methods
