...
Extract fundamental frequency (
  F0) from speech or sustained tones using object methods:
...
The `pladdrr` package provides ggplot2-based visualization functions (
  `autoplot(
    )`/`autolayer()` methods, see `vignette("autoplot-autolayer")`) for its analysis objects. For detailed examples of creating plots, see `vignette("visualization")`, which covers:
