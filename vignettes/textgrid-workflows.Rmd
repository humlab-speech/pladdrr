---
title: "TextGrid Workflows with speaker"
author: "speaker Package Authors"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{TextGrid Workflows with speaker}
...
tg <- TextGrid$create(
  xmin = 0,
  xmax = 10,
  tier_names = c("utterances", "words", "phones", "events"),
  point_tiers = c(FALSE, FALSE, FALSE, TRUE)  # First 3 are intervals, last is points
)
...
tg$set_interval_label(word_tier, interval_number = 1, label = "")
tg$set_interval_label(word_tier, interval_number = 2, label = "hello")
tg$set_interval_label(word_tier, interval_number = 3, label = "world")
tg$set_interval_label(word_tier, interval_number = 4, label = "this")
tg$set_interval_label(word_tier, interval_number = 5, label = "is")
tg$set_interval_label(word_tier, interval_number = 6, label = "a")
tg$set_interval_label(word_tier, interval_number = 7, label = "test")
...
for (i in 1:n_intervals) {
  ...
  intervals_df$label[i] <- tg$get_interval_label(tier_idx, i)
}
...
# Load large file
tg_file <- system.file("extdata", "benchmarkdata10min.TextGrid", package = "speaker")
...
ggplot(intervals_df, aes(x = duration)) +
...
# Label frequency
label_counts <- table(intervals_df$label)
label_df <- data.frame(
  label = names(label_counts),
  count = as.numeric(label_counts)
)

ggplot(label_df, aes(x = reorder(label, count), y = count)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +
  labs(
    title = "Annotation Label Frequency",
    x = "Label",
    y = "Count"
  ) +
  theme_minimal()
...
The **speaker** package provides comprehensive TextGrid support:
