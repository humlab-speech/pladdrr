...
6. **From Excitation** (
  `excitation$to_formant`) - Perceptual-based extraction, via `Spectrum$to_excitation()` or `Cochleagram$to_excitation()`
...
`to_formant_willems(
  )` and `to_formant_sl()` (split-Levinson) provide alternative
LPC-based extraction. `to_formant_robust(
  )` iteratively refits Burg while down-weighting
outlier frames (`num_std_dev`,
  `max_iterations`). Burg (recommended) or Keep All cover
