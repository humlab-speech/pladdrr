...
pladdrr selects its SIMD instruction set at **compile time**, and this is the
single most important thing to understand about the numbers below:

CRAN policy forbids `-march=native` and `-mavx2`, and pladdrr contains no
runtime CPU dispatch. A binary built the ordinary way therefore uses only what
the compiler guarantees for the target ABI:

- **arm64 (Apple Silicon, aarch64 Linux):** NEON, 2 doubles per vector.
- **x86_64:** SSE2, 2 doubles per vector. **AVX2 and AVX-512 code paths are
  never reached**, whatever the CPU supports, unless you rebuild from source
  with your own `~/.R/Makevars` adding `-march=native`.
...
Honestly: on arm64, not much. Measured on an M1 Pro (1 s signal,
single-threaded), toggling `pladdrr_simd()` moves whole-routine wall time by
between -1% and +6%:

| Routine | SIMD off | SIMD on | Gain |
|---|---:|---:|---:|
| `to_pitch()` | 5.73 ms | 5.80 ms | 0.99x |
| `to_formant_burg()` | 7.64 ms | 7.76 ms | 0.98x |
| `to_intensity()` | 0.55 ms | 0.52 ms | 1.06x |
| `to_spectrogram()` | 2.05 ms | 2.00 ms | 1.03x |
| `to_mfcc()` | 2.91 ms | 2.90 ms | 1.00x |
| `calculate_cpps_fast()` | 974 ms | 970 ms | 1.00x |

The reason is that whole-analysis runtime is dominated by work SIMD does not
touch — for CPPS, roughly 94% is the per-frame robust trend fit, which is
branchy selection code. SIMD pays off in the narrow kernels it covers
(vector statistics, window application), not in end-to-end analysis time.

Where it *is* visible is in the batch statistics bridges, which operate on
long plain vectors:

| Operation (1e6 doubles) | base R | pladdrr bridge | Gain |
|---|---:|---:|---:|
| mean | 1.98 ms | 0.48 ms | 4.1x |
| range | 5.15 ms | 0.33 ms | 15.6x |
| sd | 3.35 ms | 1.16 ms | 2.9x |
...
`pladdrr_simd()` toggle does not affect it. Run the chunk above on your own
hardware rather than relying on a quoted figure.
...
Beyond SIMD, pladdrr provides **advanced performance APIs** for users who need maximum speed in batch processing workflows.

### Batch Statistics API (10-50x faster)

When calculating statistics over multiple time intervals, use batch functions instead of repeated R method calls:

```{r batch-statistics, eval=FALSE}
# Create pitch object
sound <- Sound$new("speech.wav")
...
# SLOW: Individual R method calls (100 * 6 = 600 R↔C++ boundary crossings)
for (i in 1:100) {
...
# FAST: Single C++ call (1 boundary crossing)
stats_matrix <- pitch_get_statistics_batch(
  pitch$.xptr,
  from_times,
  to_times,
  metrics,
...
```{r intensity-batch, eval=FALSE}
intensity <- sound$to_intensity()
intensity_stats <- intensity_get_statistics_batch(
  intensity$.xptr,
  from_times,
  to_times,
  c("min", "max", "mean"),
...
For two-pass pitch analysis, get quartiles and adaptive range in single call:

```{r adaptive-range, eval=FALSE}
# Create initial pitch estimate
pitch <- sound$to_pitch_cc(pitch_floor = 50, pitch_ceiling = 800)

# FAST: Get Q1, Q3, and adaptive range in one call
...
### XPtr Window Functions (70x faster with RcppXPtrUtils)

For custom window functions or transforms, use compiled C++ functions:

```{r xptr-windows, eval=FALSE}
...
| API | Use When | Speedup |
|-----|----------|---------|
| `pitch_get_statistics_batch()` | >10 intervals, multiple metrics | 10-50x |
| `intensity_get_statistics_batch()` | >10 intervals, multiple metrics | 10-50x |
| `calculate_cpps_fast()` | AVQI, voice quality analysis | 1.5-2x |
| `pitch_get_adaptive_range()` | Two-pass pitch analysis | 3.6x |
| `apply_window_xptr()` | Custom windowing on large files | 70x |
| `apply_transform_xptr()` | Custom DSP on large files | 70x |

### Performance Comparison Summary

| Scenario | Standard API | Advanced API | Speedup |
|----------|-------------|--------------|---------|
| AVQI v3.01 (single file) | 6.0s | 4.0s | 1.5x |
| VUV analysis | 0.36s | 0.10s | 3.6x |
| 100 pitch intervals, 6 metrics | 0.6s | 0.01s | 60x |
| Custom window, 1M samples | 3.5s | 0.05s | 70x |
