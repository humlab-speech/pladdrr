...
The safest way to create a KlattGrid is with `klattgrid_create_from_vowel()`:
...
```{r empty-grid-warning, error=TRUE}
...
**Always use `klattgrid_create_from_vowel()` or `klattgrid_create_example()` instead.**
...
**Workaround**: Always use `klattgrid_create_from_vowel()` or `klattgrid_create_example()`

**Example of safe usage:**
```{r safe-usage, error=TRUE}
# safe
kg <- klattgrid_create_from_vowel(
  duration = 0.5, f0start = 120,
  f1 = 800, b1 = 80, f2 = 1200, b2 = 120, f3 = 2500, b3 = 150
...
1. **Always use `klattgrid_create_from_vowel()`** for vowel synthesis
...
**Solution**: Use `klattgrid_create_from_vowel()` instead of empty grid
...
- pladdrr documentation: `?klattgrid_create_from_vowel`
