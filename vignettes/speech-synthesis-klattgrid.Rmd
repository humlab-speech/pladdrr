...
KlattGrid is pladdrr's implementation of the Klatt cascade/parallel formant synthesizer - a powerful tool for generating synthetic speech from parametric descriptions. Unlike recording or playing back audio, KlattGrid lets you **create speech from scratch** by specifying:
...
1. **Perfect control** - Change F1 independently of F2, impossible in human speech
2. **Parametric variation** - Generate continua (e.g., /i/ to /u/ in 10 steps)
3. **Impossible voices** - Male formants + female pitch
4. **Reproducibility** - Exact same stimulus across sessions

### The Klatt Synthesizer

Dennis Klatt's 1980 formant synthesizer remains the gold standard for parametric speech synthesis. It models the vocal tract as a cascade of resonant filters (formants) excited by a periodic source (voicing) or noise.
...
kg$add_formant_frequency_point(1, 0.5, 280)   # F1: 730 → 280 Hz
kg$add_formant_frequency_point(2, 0.5, 2250)  # F2: 1090 → 2250 Hz
kg$add_formant_frequency_point(3, 0.5, 2890)  # F3: 2440 → 2890 Hz
...
```{r spectrogram, fig.width=8, fig.height=5}
# Synthesize /a/ vowel
kg <- KlattGrid_createFromVowel(
  duration = 0.5, f0start = 120,
  f1 = 730, b1 = 80,
...
formant <- sound$to_formant_burg(formant_ceiling = 5500)
formant_df <- as.data.frame(formant) %>%
  filter(formant %in% 1:3)
...
autoplot(spec) +
  geom_line(
    data = formant_df,
    aes(time, frequency, color = factor(formant)),
...
f1_mean <- mean(df_real[df_real$formant == 1, "frequency"], na.rm = TRUE)
f2_mean <- mean(df_real[df_real$formant == 2, "frequency"], na.rm = TRUE)
f3_mean <- mean(df_real[df_real$formant == 3, "frequency"], na.rm = TRUE)
...
spec_real <- sound_real$to_spectrogram(maximum_frequency = 5000)
spec_synth <- sound_synth$to_spectrogram(maximum_frequency = 5000)
...
### 1. Empty Grid Segfault

**Issue**: Direct synthesis from empty `KlattGrid()` crashes
...
| Batch synthesis | ✅ R loops | ❌ Manual |
| Parametric experiments | ✅ Easy continua | ❌ Tedious |
...
Existing R packages (e.g., `phonTools`) use simpler synthesis methods. KlattGrid provides research-grade formant synthesis matching Praat's quality.

## Summary

**KlattGrid is ideal for:**
...
- `add_formant_frequency_point()` - Formant transitions
