---
title: "Migration Guide (v3.0)"
description: |
  Complete migration guide for pladdrr v3.0 breaking changes and deprecated functions.
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Migration Guide (v3.0)}
...
**Notes:**
- Wraps your existing per-file analysis code with no other changes
...
**Note:** `get_formants_at_times()` returns a list with F1, F2, F3, F4:
...
If you work with Praat unit codes directly, use the new `unit_to_code()` utility (internal, not exported - call with `pladdrr:::`):
...
4. **Measure your own workload** - Use `system.time()`/`microbenchmark` to see whether a change helps your pipeline
...
|---------|----------|--------------|
| Parallel processing | `analyze_files_parallel()` | Distributes file-level work across cores |
| Direct API | `to_*_direct()` | Skips R6 object dispatch |
| Batch stats | `get_pitch_stats_direct()` | Returns all statistics in one call |
| Combined ops | `sound_extract_and_pitch()` | Combines extraction and analysis in one call |
...
3. **Adopt new APIs** - parallel processing and Direct API skip R6 dispatch overhead
4. **Read new docs** - see `vignette("performance-optimization")`
