...
make_test_mfcc <- function(num_coefficients = 12) {
  snd <- generate_sine_wave(frequency = 150, duration = 0.3,
    sampling_rate = 16000,
                             amplitude = 0.8)
  snd$to_mfcc(num_coefficients = num_coefficients, analysis_width = 0.02,
    time_step = 0.005)
}
...
make_test_lfcc <- function(num_coefficients = 12) {
  ...
  lpc <- snd$to_lpc_burg(prediction_order = num_coefficients,
    analysis_width = 0.02,
                         time_step = 0.005)
...
test_that(
  "MFCC frame-level queries handle out-of-range frames per their own C++ guards", {
  mfcc <- make_test_mfcc(12)
  n_frames <- mfcc$get_number_of_frames()
...
test_that(
  "Sound$to_mfcc errors (rather than crashing) when analysis_width exceeds the signal duration", {
  ...
  snd <- generate_sine_wave(frequency = 150, duration = 0.05,
    sampling_rate = 16000,
                             amplitude = 0.8)
  expect_error(
    snd$to_mfcc(num_coefficients = 12, analysis_width = 5.0, time_step = 0.005))
})
...
test_that(
  "LFCC frame-level queries handle out-of-range frames per their own C++ guards", {
  lfcc <- make_test_lfcc(12)
  n_frames <- lfcc$get_number_of_frames()
