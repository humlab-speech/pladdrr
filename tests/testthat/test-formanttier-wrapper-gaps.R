...
test_that("empty FormantTier getters and helpers", {
  ...
  expect_equal(ft$get_start_time(), 0, tolerance = sqrt(.Machine$double.eps))
  expect_equal(ft$get_end_time(), 1, tolerance = sqrt(.Machine$double.eps))
  expect_equal(ft$get_duration(), 1, tolerance = sqrt(.Machine$double.eps))
