...
test_that("SIMD tone generation handles different frequencies", {
  ...
  # Low frequency
  sound_low <- Sound$create_tone(1.0, 44100, 110, 0.5)
  ...
  # High frequency
  sound_high <- Sound$create_tone(1.0, 44100, 1000, 0.5)
...
test_that("SIMD tone generation handles different amplitudes", {
  ...
  # Low amplitude
  sound_quiet <- Sound$create_tone(1.0, 44100, 440, 0.1)
  
  mat_quiet <- sound_quiet$as_matrix()
  expect_equal(max(abs(mat_quiet)), 0.1, tolerance = 0.01)
  
  # High amplitude
  sound_loud <- Sound$create_tone(1.0, 44100, 440, 0.9)
...
test_that("SIMD tone generation handles different durations", {
  ...
  # Short
  sound_short <- Sound$create_tone(0.1, 44100, 440, 0.5)
  
  expect_equal(sound_short$get_duration(), 0.1, tolerance = 0.01)
  
  # Medium
  sound_medium <- Sound$create_tone(5.0, 44100, 440, 0.5)
  
  expect_equal(sound_medium$get_duration(), 5.0, tolerance = 0.01)
  
  # Long
  sound_long <- Sound$create_tone(30.0, 44100, 440, 0.5)
...
test_that("SIMD tone generation is deterministic", {
  ...
  sound1 <- Sound$create_tone(1.0, 44100, 440, 0.5)
  
  sound2 <- Sound$create_tone(1.0, 44100, 440, 0.5)
...
test_that("SIMD tone generation performance is reasonable", {
  ...
  time_taken <- system.time({
    sound <- Sound$create_tone(10.0, 44100, 440, 0.5)
  })["elapsed"]
