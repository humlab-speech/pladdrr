# test-voice-report.R - Regression test for PointProcess$voice_report() arg-order bug

test_that("voice_report() returns sane, in-range statistics", {
  sound <- Sound$create_tone(frequency = 150, duration = 1.0, sampling_rate = 16000)
  ...
  expect_true(report$fraction_unvoiced_frames >= 0 && report$fraction_unvoiced_frames <= 1)
  expect_true(report$degree_of_voice_breaks >= 0 && report$degree_of_voice_breaks <= 1)

  # A clean synthetic tone should be mostly voiced with low jitter.
  expect_true(report$fraction_unvoiced_frames < 0.5)
  expect_true(report$jitter_local >= 0 && report$jitter_local < 0.1)
...
test_that("voice_report() accepts silence_threshold/voicing_threshold and rejects bad types", {
  ...
  report <- pp$voice_report(sound, pitch, silence_threshold = 0.05, voicing_threshold = 0.5)
  expect_true(is.list(report))
