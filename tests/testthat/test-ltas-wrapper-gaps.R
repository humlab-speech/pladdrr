...
test_that("ltas_average handles lists, validates, and averages", {
  lt1 <- ltas_fixture()
  lt2 <- Sound$create_tone(frequency = 330, duration = 0.3, sampling_rate = 16000)$to_ltas()
...
test_that("spectral trend print covers the linear model", {
  trend <- ltas_fixture()$report_spectral_trend(fmin = 100, fmax = 5000, frequency_scale = "linear")
