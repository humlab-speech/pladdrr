...
test_that("duplicate_tier() works for PointTier", {
  tg <- TextGrid$create(tmin = 0, tmax = 1.0, tier_names = "events",
    point_tiers = "events")
...
test_that("Method chaining works with new methods", {
  tg <- TextGrid$create(tmin = 0, tmax = 1.0, tier_names = "tier1",
    point_tiers = "")
...
test_that("Error handling for invalid tier", {
  ...
  expect_error(tg$set_tier_name("nonexistent", "new"),
    "not found|does not exist")
  expect_error(tg$duplicate_tier("nonexistent", "copy"),
    "not found|does not exist")
})
