...
test_that(
  "FormantTier speckle (default) and line styles both work and use the right geom", {
  ...
  # roxygen is stale/aspirational doc text, not an implemented method — verified
  #  during SDD Task 10,
  ...
  expect_true(
    any(vapply(p_line$layers, function(l) inherits(l$geom, "GeomLine"),
                         logical(1))))
...
test_that(
  "KlattGrid autoplot produces non-empty data after Task 2's formantType fix", {
...
test_that(
  "KlattGrid with an unrecognized formant_type errors clearly instead of silently returning NA", {
  kg <- KlattGrid(0, 0.3, numberOfFormants = 3)
