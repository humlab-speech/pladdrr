...
test_that("autolayer.Sound and autolayer.Intensity respect from_time/to_time", {
  sound <- sound_fixture()
  p <- ggplot2::ggplot() + ggplot2::autolayer(sound, from_time = 0.05,
    to_time = 0.15)
  expect_s3_class(p, "ggplot")

  intensity <- sound$to_intensity()
  p2 <- ggplot2::ggplot() + ggplot2::autolayer(intensity, from_time = 0.05,
    to_time = 0.15)
...
test_that(
  "autolayer.Formant returns NULL once max_formant/time filtering empties the data", {
  formant <- sound_fixture()$to_formant()
  expect_null(ggplot2::autolayer(formant, from_time = 100, to_time = 200))
})

test_that(
  "autoplot.Spectrogram/autolayer.Spectrogram respect from_freq/to_freq", {
  ...
  p2 <- ggplot2::ggplot() + ggplot2::autolayer(spectrogram, from_time = 0.05,
    to_time = 0.15,
...
test_that(
  "autoplot.Spectrum/autolayer.Spectrum render, respect from_freq/to_freq, and support log_freq", {
  spectrum <- sound_fixture()$to_spectrum()
  p <- ggplot2::autoplot(spectrum, from_freq = 100, to_freq = 3000,
    log_freq = TRUE)
  expect_s3_class(p, "ggplot")
  expect_true(all(p$data$frequency >= 100 & p$data$frequency <= 3000))

  p2 <- ggplot2::ggplot() + ggplot2::autolayer(spectrum, from_freq = 100,
    to_freq = 3000)
  expect_s3_class(p2, "ggplot")
})

test_that(
  "autoplot.Ltas/autolayer.Ltas render, respect from_freq/to_freq, and support log_freq", {
  ltas <- sound_fixture()$to_ltas(bandwidth = 100)
  p <- ggplot2::autoplot(ltas, from_freq = 100, to_freq = 3000, log_freq = TRUE)
  expect_s3_class(p, "ggplot")
  expect_true(all(p$data$frequency >= 100 & p$data$frequency <= 3000))

  p2 <- ggplot2::ggplot() + ggplot2::autolayer(ltas, from_freq = 100,
    to_freq = 3000)
  expect_s3_class(p2, "ggplot")
})
...
test_that("autolayer.Harmonicity respects from_time/to_time", {
  ...
  p <- ggplot2::ggplot() + ggplot2::autolayer(hnr, from_time = 0.05,
    to_time = 0.15)
  expect_s3_class(p, "ggplot")
})

test_that(
  "autoplot.PointProcess/autolayer.PointProcess render, respect from_time/to_time, and warn/return NULL when empty", {
  pp <- sound_fixture()$to_point_process_periodic_cc(pitch_floor = 75,
    pitch_ceiling = 500)
  ...
  p3 <- ggplot2::ggplot() + ggplot2::autolayer(pp, from_time = 0.05,
    to_time = 0.15,
                                                ymin = 0.2, ymax = 0.8)
  ...
  expect_warning(p_empty <- ggplot2::autoplot(pp0),
    "PointProcess has no points")
  expect_s3_class(p_empty, "ggplot")
  expect_null(ggplot2::autolayer(pp0))
})

test_that(
  "autoplot.PowerCepstrum/autolayer.PowerCepstrum respect from_quefrency/to_quefrency and warn/return NULL when empty", {
  pcep <- sound_fixture()$to_spectrum()$to_power_cepstrum()
  p <- ggplot2::autoplot(pcep, from_quefrency = 0.001, to_quefrency = 0.01,
    mark_peak = TRUE)
  expect_s3_class(p, "ggplot")
  expect_true(all(p$data$quefrency >= 0.001 & p$data$quefrency <= 0.01))

  p2 <- ggplot2::ggplot() + ggplot2::autolayer(pcep, from_quefrency = 0.001,
    to_quefrency = 0.01)
  expect_s3_class(p2, "ggplot")
...
test_that(
  "autolayer.TextGrid resolves tiers by name/index, filters by from_time/to_time, renders point tiers, and errors on unknown/out-of-range tiers", {
  ...
  layer3 <- ggplot2::autolayer(tg, tier = "words", from_time = 0.6,
    to_time = 1.0)
  ...
  layer5 <- ggplot2::autolayer(tg2, tier = "tones", from_time = 0.5,
    to_time = 1.0)
