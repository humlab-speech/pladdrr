...
test_that("Pitch get_statistics returns all default metrics", {
  ...
  expect_true(
    all(
      c("mean", "stdev", "min", "max", "median", "q1", "q3") %in% names(stats)))
...
test_that(
  "Pitch get_statistics accepts every metric alias, including count_voiced", {
  pitch <- pitch_fixture()
  stats <- pitch$get_statistics(
    metrics = c(
...
test_that(
  "Pitch get_time_of_minimum and get_time_of_maximum return times within the domain", {
  pitch <- pitch_fixture()
...
test_that(
  "Pitch get_mean_intensity averages frame intensity over an explicit range", {
  pitch <- pitch_fixture()
...
test_that(
  "Pitch get_mean_intensity(0, 0) is NA (unlike get_mean/get_minimum, it has no whole-range special case)", {
...
test_that(
  "Pitch get_intensities_vector returns one frame intensity per frame", {
  pitch <- pitch_fixture()
...
test_that(
  "Pitch as_data_frame with both include_strength and include_intensity returns all five columns", {
  pitch <- pitch_fixture()
  df <- pitch$as_data_frame(include_strength = TRUE, include_intensity = TRUE)

  expect_true(
    all(
      c("time", "frequency", "voiced", "strength",
        "intensity_db") %in% names(df)))
  expect_equal(nrow(df), pitch$get_number_of_frames(),
    tolerance = sqrt(.Machine$double.eps))
})

test_that(
  "Pitch as_data_frame with only include_intensity returns intensity but not strength", {
  pitch <- pitch_fixture()
  df <- pitch$as_data_frame(include_strength = FALSE, include_intensity = TRUE)

  expect_true(
    all(c("time", "frequency", "voiced", "intensity_db") %in% names(df)))
...
test_that(
  "Pitch save errors cleanly when the target directory does not exist", {
  pitch <- pitch_fixture()
  expect_error(
...
test_that(
  "Pitch to_textgrid_silences on a fully-voiced tone returns one sounding interval", {
  pitch <- pitch_fixture()
...
test_that(
  "Pitch to_textgrid_silences splits silence/tone/silence into three labeled intervals", {
  sr <- 44100
  dur_tone <- 0.6
  dur_sil <- 0.4
  ...
  tg <- pitch$to_textgrid_silences(min_silent_duration = 0.05,
...
test_that(
  "Pitch to_textgrid_silences with a very high min_sounding_duration collapses short voiced spans away", {
  sr <- 44100
  dur_tone <- 0.6
  dur_sil <- 0.4
  t <- seq(0, dur_tone, length.out = round(sr * dur_tone))
