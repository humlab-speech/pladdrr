...
make_test_cepstrum <- function() {
  sound <- Sound$create_tone(duration = 0.5, frequency = 200,
    sampling_rate = 44100)
...
test_that("get_quefrency_of_peak works for all interpolation methods", {
  ...
  for (interp in c("parabolic", "none", "cubic")) {
    q <- cepstrum$get_quefrency_of_peak(interpolation = interp, qmin = 0.003,
      qmax = 0.04)
...
test_that(
  "get_value_at_quefrency works for all interpolation/unit combinations", {
  cepstrum <- make_test_cepstrum()
  q <- cepstrum$get_quefrency_of_peak()

  for (interp in c("linear", "cubic")) {
    for (unit in c("dB", "linear")) {
      val <- cepstrum$get_value_at_quefrency(q, interpolation = interp,
        unit = unit)
...
test_that(
  "get_value_at_quefrency dB and linear units agree and are plausible", {
  ...
  db_value <- cepstrum$get_value_at_quefrency(q1, interpolation = "linear",
    unit = "dB")
  linear_value <- cepstrum$get_value_at_quefrency(q1,
    interpolation = "linear", unit = "linear")
