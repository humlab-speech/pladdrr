...
create_voiced_signal <- function(duration = 1.0, f0 = 150, sr = 44100) {
    Sound$create_tone(duration = duration, sampling_rate = sr, frequency = f0,
      amplitude = 0.8)
}
...
test_that(
  "extract_voiced_segments_ultra matches Praat on a constant-amplitude tone", {
    ...
    # 30 ms window's zero-crossing walk runs off the window edge, so
    #  `checkZeros`
...
test_that("ZCR calculation uses interpolated zero crossings", {
    ...
    sound <- Sound$create_tone(duration = 0.5, sampling_rate = 44100,
      frequency = freq)
    ...
    expect_lt(abs(mean_zcr - expected_zcr) / expected_zcr, 0.2,
      label = sprintf("ZCR %.1f should be ~%.1f Hz", mean_zcr,
      expected_zcr))
})

test_that(
  "extract_voiced_segments_ultra always returns at least Praat's 1 ms seed", {
...
test_that("extract_voiced_segments_ultra works with different sampling rates", {
    ...
    for (sr in sr_list) {
        sound <- Sound$create_tone(duration = 1.0, sampling_rate = sr,
          frequency = 150)
...
test_that("extract_voiced_segments_ultra handles edge cases", {
    # Very short signal
    short_sound <- Sound$create_tone(duration = 0.05, sampling_rate = 44100,
      frequency = 150)
...
test_that("windows are kept, and the kept count matches Praat, at 137 Hz", {
    ...
    # inside the window, and windows are actually kept -- which is what
    #  exercises
...
test_that("CPPS calculation on ultra-extracted segments is reasonable", {
    skip_if_not(
      exists("calculate_cpps_ultra", where = asNamespace("pladdrr"),
        inherits = FALSE))
    ...
    if (is.finite(cpps)) {
        expect_gt(cpps, 0,
          label = sprintf("CPPS %.2f dB should be > 0 for periodic signal",
            cpps))
    }
})
...
test_that(
  "extract_voiced_segments_ultra rejects a null external pointer at the C++ layer", {
...
test_that(
  "extract_voiced_segments_ultra returns the minimal 1ms seed when nothing is sounding", {
  ...
  # paths.
  silence <- Sound$create_tone(frequency = 0, duration = 0.5,
    sampling_rate = 16000)
  result <- suppressWarnings(
    extract_voiced_segments_ultra(silence, version = "v3.01"))
  expect_s3_class(result, "Sound")
  expect_equal(result$get_total_duration(), 0.001, tolerance = 1e-9)
})

test_that(
  "extract_voiced_segments_ultra concatenates multiple sounding regions on real speech", {
