...
test_that("extract_pitch()/get_*_pitch() delegate to the R6 Pitch API", {
  ...
  expect_warning(
    pitch <- extract_pitch(sound, pitch_floor = 75,
      pitch_ceiling = 600), "deprecated")
  expect_s3_class(pitch, "Pitch")

  expect_warning(val <- get_pitch_at_time(pitch, 0.25, unit = "Hz"),
    "deprecated")
  expect_equal(val, pitch$get_value_at_time(0.25, "hertz"),
    tolerance = sqrt(.Machine$double.eps))

  expect_warning(mean_val <- get_mean_pitch(pitch), "deprecated")
  expect_equal(mean_val, pitch$get_mean(0, 0, "hertz"),
    tolerance = sqrt(.Machine$double.eps))

  expect_warning(min_val <- get_min_pitch(pitch, time_range = c(0.1, 0.3)),
    "deprecated")
  expect_equal(min_val,
    pitch$get_minimum(0.1, 0.3, "hertz",
      interpolate = FALSE), tolerance = sqrt(.Machine$double.eps))

  expect_warning(max_val <- get_max_pitch(pitch), "deprecated")
  expect_equal(max_val, pitch$get_maximum(0, 0, "hertz", interpolate = FALSE),
    tolerance = sqrt(.Machine$double.eps))
})
...
test_that(
  "extract_intensity()/get_*_intensity() delegate to the R6 Intensity API", {
  sound <- tone_sound()
  ...
  expect_equal(val, intensity$get_value_at_time(0.25),
    tolerance = sqrt(.Machine$double.eps))

  expect_warning(mean_val <- get_mean_intensity(intensity), "deprecated")
  expect_equal(mean_val, intensity$get_mean(0, 0),
    tolerance = sqrt(.Machine$double.eps))

  expect_warning(
    min_val <- get_min_intensity(intensity,
      time_range = c(0.1, 0.3)), "deprecated")
  expect_equal(min_val,
    intensity$get_minimum(0.1, 0.3,
      interpolation = "none"), tolerance = sqrt(.Machine$double.eps))

  expect_warning(max_val <- get_max_intensity(intensity), "deprecated")
  expect_equal(max_val, intensity$get_maximum(0, 0, interpolation = "none"),
    tolerance = sqrt(.Machine$double.eps))

  expect_warning(sd_val <- get_sd_intensity(intensity), "deprecated")
  expect_equal(sd_val, intensity$get_standard_deviation(0, 0),
    tolerance = sqrt(.Machine$double.eps))
})
...
test_that("create_sound()/get_*() delegate to the R6 Sound API", {
  expect_warning(sound <- create_sound(c(-1, 0, 1), sampling_rate = 8000),
    "deprecated")
  ...
  expect_equal(sr, sound$get_sampling_frequency(),
    tolerance = sqrt(.Machine$double.eps))

  expect_warning(nch <- get_n_channels(sound), "deprecated")
  expect_equal(nch, sound$get_number_of_channels(),
    tolerance = sqrt(.Machine$double.eps))

  expect_warning(ns <- get_n_samples(sound), "deprecated")
  expect_equal(ns, sound$get_number_of_samples(),
    tolerance = sqrt(.Machine$double.eps))
})

test_that(
  "read_sound() delegates to Sound$new() and supports channel extraction", {
