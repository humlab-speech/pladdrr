test_that("calculate_cpps_fast() formals match the r6 profile in .cpps_profiles", {
  ...
  expect_equal(f$pre_emphasis_from, 50, tolerance = sqrt(.Machine$double.eps))
})
...
test_that("calculate_cpps_ultra() formals match documented defaults", {
  ...
  expect_equal(f$pitch_floor, 60, tolerance = sqrt(.Machine$double.eps))
  expect_equal(f$pitch_ceiling, 333.3)
  expect_true(f$subtract_trend)
  expect_identical(f$line_type, "straight")
  expect_identical(f$fit_method, "robust")
  expect_identical(f$interpolation, "parabolic")
  expect_equal(f$time_step, 0.002)
  expect_equal(f$max_quefrency, 0.04)
  expect_equal(f$tolerance, 0.05)
  expect_equal(f$tilt_line_quefrency, 0.003)
  expect_equal(f$pre_emphasis_from, 50, tolerance = sqrt(.Machine$double.eps))
  expect_equal(f$max_frequency, 5000, tolerance = sqrt(.Machine$double.eps))
})
