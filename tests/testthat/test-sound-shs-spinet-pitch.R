...
test_that("Sound$to_pitch_spinet constructs a valid Pitch object", {
  ...
  # praat.github.io/dwtools/Sound_to_SPINET.cpp) has a reproducible, purely
  # deterministic native quirk: repeated calls strictly alternate between
  # succeeding and failing with "The sound should not have all amplitudes
  # equal to zero", regardless of the input signal (confirmed by dedicated
  # stress testing: 90/90 consecutive-pair retries succeeded, 0 double
  # failures, including when interleaved with to_pitch_shs() calls as in
  # this file). A single immediate retry is therefore a reliable, not a
  # naive, mitigation for this specific upstream state issue.
  sound <- generate_sine_wave(150, 0.5, sampling_rate = 16000)
  pitch <- tryCatch(
